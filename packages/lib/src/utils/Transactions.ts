...
export async function isGanacheNode(): Promise<boolean> {
  const nodeVersion: string = await getNodeVersion();
  const matches = nodeVersion.match(/TestRPC/);
...
export async function awaitConfirmations(transactionHash: string, confirmations: number = 12, interval: number = 1000, timeout: number = (10 * 60 * 1000)): Promise<any> | never {
  ...
  const getTxBlock = () => (ZWeb3.getTransactionReceipt(transactionHash).then((r: Web3TransactionReceipt) => r.blockNumber));
...
export async function hasBytecode(address: string): Promise<boolean> {
  const bytecode: string = await ZWeb3.getCode(address);
