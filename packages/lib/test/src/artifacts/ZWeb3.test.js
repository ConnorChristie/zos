...
contract.only('ZWeb3', accounts => {
  ...
  describe('when initialized', function () {
    ...
    it('can builds an object wrapper of a contract', async function () {
      const contractClass = ZWeb3.contract(this.impl.constructor.abi)

      const instance = await new Promise((resolve, reject) => {
        contractClass.new({ data: this.impl.constructor.bytecode, ...Contracts.getArtifactsDefaults() }, (error, instance) => {
      ...
      const version = await new Promise((resolve, reject) =>
        instance.version(Contracts.getArtifactsDefaults(), (error, result) => error ? reject(error) : resolve(result))
      )
